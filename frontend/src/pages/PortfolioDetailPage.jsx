...
export default function PortfolioDetailPage() {
    ...
    const handleAddHolding = async (e) => {
        ...
        try {
            ...
            if (newHolding.purchaseDate) {
                payload.purchaseDate = new Date(newHolding.purchaseDate).toISOString();
            }
    ...
    const handleTransaction = async (e) => {
        ...
        try {
            ...
            if (txForm.executedAt) {
                payload.executedAt = new Date(txForm.executedAt).toISOString();
            }
            ...
            if (parseFloat(updatedHolding.quantity) === 0) {
                ...
                setHoldings(holdings.map(h => h.id === txModal.holdingId ? updatedHolding : h));
            }
    ...
    const toggleTransactions = async (holdingId) => {
        ...
        if (!holdingTransactions[holdingId]) {
            try {
                const response = await fetch(`http://localhost:8081/api/portfolios/holdings/${holdingId}/transactions`, {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
                });
                if (response.ok) {
                    const data = await response.json();
                    setHoldingTransactions(prev => ({ ...prev, [holdingId]: data }));
                }
            } catch { }
        }
    };
